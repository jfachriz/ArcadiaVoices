...
def main():
  ...
  vst3_bundle = build_dir + "\\ArcadiaVoices.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\ArcadiaVoices Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\ArcadiaVoices Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\build-win\\manual\\ArcadiaVoices manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\ArcadiaVoices.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "ArcadiaVoices.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\ArcadiaVoices_x64.exe",
      projectpath + "\\build-win\\ArcadiaVoices_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\ArcadiaVoices_x64.clap", "ArcadiaVoices_x64.clap"),
      (projectpath + "\\build-win\\ArcadiaVoices_ARM64EC.clap", "ArcadiaVoices_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\ArcadiaVoices.clap", "ArcadiaVoices_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\ArcadiaVoices.clap", "ArcadiaVoices_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\ArcadiaVoices-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\ArcadiaVoices-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\ArcadiaVoices-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\ArcadiaVoices-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\ArcadiaVoices-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\ArcadiaVoices-clap_ARM64EC.pdb"
  ]
