...
def main():
  ...
  # WebView UI (built React app - see src/ + vite.config.ts, output copied here via `npm run build`)
  # LoadIndexHtml()/LoadFile() expects the bundled HTML at Resources/web/index.html
  webSrc = os.path.join(projectpath, "resources", "web")
  if os.path.exists(webSrc):
    webDst = os.path.join(dst, "web")
    print("copying WebView UI from " + webSrc + " to " + webDst)
    if os.path.exists(webDst):
      shutil.rmtree(webDst)
    shutil.copytree(webSrc, webDst)
  else:
    print("WARNING: " + webSrc + " not found - the plugin UI will fail to load. Run `npm run build` at the project root and copy dist/ to iPlug2OOS/ArcadiaVoices/resources/web/ first.")
